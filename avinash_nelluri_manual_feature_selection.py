...
for split_ratio in split_ratios:
    ...
    smote = SMOTE()
    X_resampled, y_resampled = smote.fit_resample(train_pd[feature_columns], train_pd['label'])
    train_resampled = pd.DataFrame(X_resampled, columns=feature_columns)
    train_resampled['label'] = y_resampled
    
    # Convert back to Spark DataFrame
    train_df_resampled = spark.createDataFrame(train_resampled)
    
    # Train and evaluate Logistic Regression
    lr = LogisticRegression(maxIter=100, regParam=0.01, elasticNetParam=0.8, family="multinomial", featuresCol="selectedFeatures", labelCol="label")
    train_and_evaluate(lr, train_df_resampled, test_df, "Logistic Regression")
    
    # Train and evaluate Decision Tree
    dt = DecisionTreeClassifier(featuresCol="selectedFeatures", labelCol="label")
    train_and_evaluate(dt, train_df_resampled, test_df, "Decision Tree")
    
    # Train and evaluate Random Forest
    rf = RandomForestClassifier(featuresCol="selectedFeatures", labelCol="label", numTrees=100, maxDepth=10)
    train_and_evaluate(rf, train_df_resampled, test_df, "Random Forest")
    
    # Train and evaluate Naive Bayes
    nb = NaiveBayes(featuresCol="selectedFeatures", labelCol="label", modelType="multinomial")
    train_and_evaluate(nb, train_df_resampled, test_df, "Naive Bayes")
    ...
    train_and_evaluate(ovr, train_df_resampled, test_df, "SVM")
